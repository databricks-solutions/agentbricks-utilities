...
dbutils.widgets.text("source_volume_path", "/Volumes/main/default/source_docs", "Source volume path")
dbutils.widgets.text("input_file", "*.pdf")
dbutils.widgets.text("output_volume_path", "/Volumes/main/default/parsed_images", "Output volume path")
dbutils.widgets.text("page_selection", "all")

catalog = dbutils.widgets.get("catalog")
schema = dbutils.widgets.get("schema")
source_volume_path = dbutils.widgets.get("source_volume_path")
parsed_table = dbutils.widgets.get("table_name")
input_file = dbutils.widgets.get("input_file")
image_output_path = dbutils.widgets.get("output_volume_path")
...
source_files = f"{source_volume_path}/{input_file}"
image_output_path = dbutils.widgets.get("output_volume_path")

print(f"Source files: {source_files}")
print(f"Parsed table: {catalog}.{schema}.{parsed_table}")
print(f"Image output path: {image_output_path}")
...
parsed_results = [row.raw_parsed for row in spark.sql(f'select * from {catalog}.{schema}.{parsed_table}').collect()]
...
def render_ai_parse_output_interactive(parsed_results):
    ...
    def get_current_document():
        """Get the currently selected document and its pages."""
