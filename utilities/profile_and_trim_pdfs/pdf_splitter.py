...
# Optional
dbutils.widgets.text("pages_per_split", "1000")
...
def make_config():
    ...
    pages_per_split = int(dbutils.widgets.get("pages_per_split"))
    config_map = {
            "catalog": catalog,
...
class SplitterConfig:
    ...
    def process_dbutils(cls, dbutil_map):
        obj = cls(
            ...
            pages_per_split=int(dbutil_map["pages_per_split"]),
        )
...
def validate_widgets(splitter_config: SplitterConfig):
    """
    ...
    if not os.path.exists(splitter_config.dest_path):
        raise ValueError(f"Ensure the destination volume and folder, {splitter_config.dest_path}, exists for your split PDFs. If it does not, you must create it.")

    if not os.path.exists(splitter_config.source_path):
        raise ValueError(f"Ensure the source volume, {splitter_config.source_path}, exists for your PDFs.")
...
SPLIT_SCHEMA = StructType([
    ...
    StructField("num_splits", IntegerType(), True),
    StructField("split_paths", ArrayType(StringType()), True),
    StructField("error", StringType(), True),
])
config_map = make_config()
SPLITTER_CONFIG = SplitterConfig.process_dbutils(config_map)
validate_widgets(SPLITTER_CONFIG)
...
class PDFSplitter:
    def __init__(self):
        """
        PDF Splitter to split large PDFs into smaller chunks.
        """
    ...
    def split(
        ...
        pages_per_split: int = 1000
    ) -> Dict:
        """Split PDF into multiple files."""
        return self._split_pdf(
            pdf_path,
            pages_per_split=pages_per_split,
            destination_path=splitter_config.dest_path,
        )
    ...
    def _split_pdf(
        ...
        """
        Internal worker: split a PDF into multiple files.
        Returns dict with split info and error if any.
        """
        result = {
            "path": pdf_path,
            "size_bytes": None,
        ...
        try:
            ...
            for split_idx in range(num_splits):
                ...
                end_page = min((split_idx + 1) * pages_per_split, total_pages)
                
                writer = PdfWriter()
                for page_idx in range(start_page, end_page):
                    writer.add_page(reader.pages[page_idx])

                # Create filename with page range: original_name_pages_1-1000.pdf
                split_filename = f"{file_base}_pages_{start_page+1}-{end_page}.pdf"
                destination_file_path = os.path.join(destination_path, split_filename)
                ...
                split_paths.append(destination_file_path)
...
def make_split_udf(splitter_config, split_schema):
    """Higher-order function returning pandas UDF to handle passing config."""
    @pandas_udf(split_schema)
    def split_udf(pdf_paths: pd.Series) -> pd.DataFrame:
        splitter = PDFSplitter()
        return pd.DataFrame([
            splitter.split(path, splitter_config=splitter_config, pages_per_split=splitter_config.pages_per_split) 
...
def process_files(splitter_config: SplitterConfig, bronze_pdf_stream: DataFrame) -> DataFrame:
    """
    Process the files.
    """
    split_udf = make_split_udf(splitter_config, SPLIT_SCHEMA)
    return (bronze_pdf_stream
            .withColumn("split_info", split_udf(bronze_pdf_stream["path"]))
            .withColumn("id", md5(concat_ws("|", col("modificationTime"), col("path"))))
            .select("id", "modificationTime", "split_info.*")
    )


def write_stream(processed_stream: DataFrame, splitter_config: SplitterConfig) -> DataFrame:
    return (
        processed_stream.writeStream
            .option("checkpointLocation", splitter_config.checkpoint_folder)
            .outputMode("append")
            .trigger(availableNow=True)
            .toTable(splitter_config.dest_table)
            .awaitTermination()
        )
...
def main(splitter_config):
    spark.sql(f"CREATE VOLUME IF NOT EXISTS {splitter_config.full_dest_volume}")
    create_subfolder(splitter_config.dest_path)
    bronze_pdf_stream = read_bronze_pdf_stream(splitter_config.source_path)
    processed_stream = process_files(splitter_config, bronze_pdf_stream)
    write_stream(processed_stream, splitter_config)
